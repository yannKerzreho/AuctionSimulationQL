...
int main() {
    ...
    std::vector<double> alpha = {0.05};
    std::vector<double> beta = {0.00002, 0.0002};
    std::vector<double> gamma = {0.99, 0.95};
    std::vector<double> epsilon = {0.25, 0.025};
    std::vector<double> opt = {10};
    std::vector<double> num_iterations = {1000000};
    ...
    for (const std::vector<double>& row : conv) {
        std::cout << "A: " << row[0] << " | B: " << row[1] << std::endl;
    }
